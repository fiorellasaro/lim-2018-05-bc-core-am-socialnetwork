...
email.addEventListener('input', () => {
  ...
  window.validateEmail(campo.value) ? replaceClassIconValid(email, valido) : replaceClassIconDanger(email, valido);
});
password.addEventListener('input', () => {
  const campo = event.target;
  addClassIcon(password, validpwd);
  window.passwordLength(campo.value) ? replaceClassIconValid(password, validpwd) : replaceClassIconDanger(password, validpwd);
});
repeatPassword.addEventListener('input', () => {
  const campo = event.target;
  menssageErrorPassword.innerHTML = '';
  addClassIcon(repeatPassword, valid);
  window.passwordRepeatValid(campo.value, password.value) ? replaceClassIconValid(repeatPassword, valid) : replaceClassIconDanger(repeatPassword, valid);
});
